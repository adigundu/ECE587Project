	...
	{	
	 ...
	 during random replacement selection) */	
      cp->sets[i].blks = CACHE_BINDEX(cp, cp->data, bindex);	
			int state_plru = cp->assoc - 1;	
			cp->sets[i].PLRU_state = (1 << state_plru) - 1;	
...
{	
	...
	int state_plru = assoc - 1;	
	int PLRU_state_mask = (1 << state_plru) - 1;	
	int i = 0;	
	int j = 0;	
	int mask;	
	for (j = 0; j < width_bindex; j++)	
...
cache_misses:
  ...
  case FIFO:
    ...
    {	
      ...
      repl = CACHE_BINDEX(cp, cp->sets[set].blks, bindex);	
		...
		break;
	case NRU:
	//Loop through to find the first block with a counter value of 1. Starting from Left going Right.
	for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
		if(blk->NRU_Counter == 1){
			repl = blk;		//When found, replace that block.
			break;		
		}
		if(blk->way_next == NULL){
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
				//All counters are at 0 so reset them back to 1:
				blk->NRU_Counter = 1;
			}
	...
	break;
  case SRRIP:
	//Loop through to find the first block with a counter value of 3. Starting from Left going Right.
	for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
		if(blk->SRRIP_Counter == 3){
			repl = blk;		//When found, replace that block.
			blk->SRRIP_Counter = 2; 
			break;		
		}
		if(blk->way_next == NULL){
			for (blk=cp->sets[set].way_head; blk; blk=blk->way_next){
				//If no 3's are found, increment all the counters:
				blk->SRRIP_Counter+=1;
			}
			blk=cp->sets[set].way_head;
			goto cache_misses;
 ...
 cache_hit: /* slow hit handler */
  /* **HIT** */
  cp->hits++;
  /* Reset the block counter to 0 on a cache hit: */
  blk->NRU_Counter = 0;		//Reset the counter;
  blk->SRRIP_Counter = 0;	//Reset the counter;
